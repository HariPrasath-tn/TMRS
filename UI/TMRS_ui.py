...
class HomeUi:
    ...
    def __init__(self, window):
        ...
        self.symptoms_radio = Radiobutton(self.left_top,
                                          ...
                                          variable=globals()['symp_var'], value="symp", command=self.prnt_it)
        ...
        self.disease_radio = Radiobutton(self.left_top, text="I know the disease name", variable=globals()['symp_var'],
                                         value="disease", command=self.prnt_it)
        self.disease_radio.grid(row=0, column=2)
        Label(self.left_top, text=" " * 7).grid(row=0, column=3)
        ...
        self.disease_label.place(x=240, y=170)
        self.disease_entry = Entry(self.right_up, textvariable=globals()['disease_name'], width="30", state="disabled",
                                   bg="white")
        self.disease_entry.place(x=370, y=174)
        self.disease_label = Label(self.right_up, text="Medicine Type : ", font=("bold", 12))
        self.disease_label.place(x=240, y=210)
        self.med_type = ttk.Combobox(self.right_up, textvariable=globals()['medicine_type'], width="30")
        self.med_type['values'] = globals()['med_type_list']
        self.med_type.place(x=370, y=214)
        self.med_confirm = Button(self.right_up, text="Confirm and Fetch Medicine", bg="orange")
        self.med_confirm.place(x=370, y=254)
        self.sug_label = Label(self.right_up, text="List of Medicine suggested : ", font=("bold", 15))
        self.sug_label.place(x=50, y=370)
        self.sug_med = Text(self.right_up, width=70, height=20)
        self.sug_med.place(x=50, y=410)
